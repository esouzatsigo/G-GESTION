...
export const generateServiceReport = async (
    ...
    // Mejorado: Carga de imágenes usando fetch + blob para evitar bloqueos de CORS en PDF
    const loadImage = async (url: string, isSignature: boolean = false): Promise<string> => {
        if (!url || typeof url !== 'string') return '';
        console.log(`[PDF] Cargando ${isSignature ? 'firma' : 'foto'}:`, url);

        try {
            // Saltamos cache si no es base64
            const finalUrl = url.includes('data:') ? url : url + (url.includes('?') ? '&' : '?') + 'cache=' + Date.now();
            ...
            if (!base64 || !isSignature) return base64;
            ...
            return new Promise<string>((resolve) => {
                ...
                img.onload = () => {
                    ...
                    for (let i = 0; i < data.length; i += 4) {
                        ...
                        const isColor = (r < 240 || g < 240 || b < 240); // No es blanco fondo

                        // Si hay tinta (por transparencia o por color)
                        if (alpha > 40 || isColor) {
                            data[i] = 0; data[i + 1] = 0; data[i + 2] = 0; // Negro
                            data[i + 3] = 255; // Opaco
                        } else {
                            data[i] = 255; data[i + 1] = 255; data[i + 2] = 255; // Blanco
                            data[i + 3] = 255;
                        }
                    }
            ...
            console.error("[PDF] Error al cargar recurso:", url, e);
            return '';
        }
    };
    ...
    drawBox(margin, currentY, colW, 28, 'DATOS DEL CLIENTE');
    doc.setTextColor(0);
    doc.setFontSize(10);
    doc.setFont('helvetica', 'bold'); doc.text('CLIENTE:', margin + 3, currentY + 14);
    doc.setFont('helvetica', 'normal'); doc.text(cliente.nombre, margin + 22, currentY + 14);
    ...
    doc.setFont('helvetica', 'normal'); doc.text(new Date(ot.fechas.solicitada).toLocaleDateString(), margin + colW + 45, currentY + 14);
    ...
    doc.text(`Documento original de H-GESTION - Generado el ${new Date().toLocaleString()}`, pageWidth / 2, 272, { align: 'center' });

    try {
        const fileName = `Servicio_OT_${ot.numero}_${sucursal.nombre.replace(/[^a-z0-9]/gi, '_')}.pdf`;
        doc.save(fileName);
        return true;
    } catch (err) {
        return false;
