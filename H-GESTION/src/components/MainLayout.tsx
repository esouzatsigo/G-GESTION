            ...
            <aside style={{
                ...
                <nav style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', flex: 1, overflowY: 'auto' }}>
                    ...
                    {(isAdmin || isCoordinador || isGerenteBA) && (
                        <NavItem icon={ClipboardList} label="Ordenes de Trabajo" active={isPathActive('/ots')} onClick={() => handleNavigation('/ots')} />
                    )}
                    {(isSupervisor || isAdmin) && (
                        <NavItem icon={ClipboardList} label="SUPERVISAR OTs CONCLUIDAS" active={isPathActive('/supervisar')} onClick={() => handleNavigation('/supervisar')} />
                    )}

                    {(isAdmin || isCoordinador || isGerenteBA || isSupervisor) && (
                        <NavItem icon={Database} label="CONSULTA DE ORDENES" active={isPathActive('/kardex')} onClick={() => handleNavigation('/kardex')} />
                    )}

                    {(isAdmin || isCoordinador) && (
                        <NavItem icon={Calendar} label="Mantenimientos Preventivos" active={isPathActive('/preventivos')} onClick={() => handleNavigation('/preventivos')} />
                    )}

                    {(isAdmin || isCoordinador) && (
                        <NavItem icon={History} label="Bitácora de Auditoría" active={isPathActive('/bitacora')} onClick={() => handleNavigation('/bitacora')} />
                    )}

                    {(isGerente || isCoordinador || isAdmin) && (
                        <NavItem icon={AlertCircle} label="Solicitar OT Correctiva" active={isPathActive('/solicitar-ot')} onClick={() => handleNavigation('/solicitar-ot')} />
                    )}
                    ...
                    {isAdmin && (
                        <>
                            ...
                            <NavItem icon={Users} label="Clientes" active={isPathActive('/clientes')} onClick={() => handleNavigation('/clientes')} />
                            <NavItem icon={Database} label="Franquicias" active={isPathActive('/franquicias')} onClick={() => handleNavigation('/franquicias')} />
                            <NavItem icon={Store} label="Sucursales" active={isPathActive('/sucursales')} onClick={() => handleNavigation('/sucursales')} />
                            <NavItem icon={HardDrive} label="Equipos" active={isPathActive('/equipos')} onClick={() => handleNavigation('/equipos')} />
                            <NavItem icon={Users} label="Usuarios" active={isPathActive('/usuarios')} onClick={() => handleNavigation('/usuarios')} />
                        </>
                    )}

                    <div style={{ marginTop: 'auto' }}>
                        <NavItem icon={Settings} label="Configuración" active={isPathActive('/config')} onClick={() => handleNavigation('/config')} />
                    </div>
                </nav>
