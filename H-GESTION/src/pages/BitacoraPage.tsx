...
export const BitacoraPage: React.FC = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                ...
                const q = query(collection(db, 'bitacora'), orderBy('otNumero', 'desc'), limit(100));
    ...
    const filtered = entries.filter(e =>
        e.otNumero.toString().includes(searchTerm) ||
        ...
        <div className="animate-fade">
            ...
            <div className="glass-card" style={{ marginBottom: '2rem' }}>
                <div style={{ position: 'relative' }}>
                    ...
                    <input
                        type="text" placeholder="Buscar por OT, usuario o acción..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)}
            ...
            {loading ? (
                ...
                <div className="scrollable-x custom-scrollbar">
                    <table style={{ width: '100%', borderCollapse: 'separate', borderSpacing: '0 0.75rem' }}>
                        ...
                        <tbody>
                            {filtered.map(entry => (
                                <tr key={entry.id} className="glass-card animate-fade" style={{ background: 'var(--bg-input)', borderLeft: '4px solid var(--primary)' }}>
                                    <td style={{ padding: '1rem' }}>
                                        <div style={{ fontWeight: '800', color: 'var(--primary)', fontSize: '0.9rem' }}>OT #{entry.otNumero}</div>
