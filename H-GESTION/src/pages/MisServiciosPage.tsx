...
export const MisServiciosPage: React.FC = () => {
    ...
    const fetchData = useCallback(async () => {
        ...
        // 1. CARGA INSTANTÁNEA DESDE CACHÉ (Memoria del técnico)
        const cachedOTs = sessionStorage.getItem('ms_ots_cache');
        const cachedSucs = sessionStorage.getItem('ms_suc_cache');
        const cachedEqs = sessionStorage.getItem('ms_eq_cache');
        ...
        if (cachedOTs && cachedSucs && cachedEqs) {
            ...
            setLoading(false);
            // No retornamos, seguimos para actualizar en segundo plano (SWR pattern)
        } else {
            setLoading(true);
        }
        ...
        try {
            ...
            // 2. CONSULTAS OPTIMIZADAS (Una sola petición por colección)
            const qOts = tenantQuery('ordenesTrabajo', user,
                where('tecnicoId', '==', user.id),
                where('fechas.solicitada', '>=', minDate)
            );

            // Solo descargamos catálogos si no están en caché o es primera vez
            ...
            const fetchedOts = results[0].docs.map((d: any) => ({ id: d.id, ...d.data() } as WorkOrder))
                .filter((ot: WorkOrder) => showCompleted 
                    ? (ot.estatus === 'Concluida' || ot.estatus === 'Finalizada')
                    : (ot.estatus !== 'Concluida' && ot.estatus !== 'Finalizada')
                );

            setOts(fetchedOts);
            sessionStorage.setItem('ms_ots_cache', JSON.stringify(fetchedOts));
            ...
            // Ajuste de índice si había catálogos extra
    ...
    useEffect(() => {
        fetchData();
    }, [user, showCompleted]); // ELIMINADO: 'searchTerm' ya no dispara descargas de red. Búsqueda local instantánea.
