    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            ...
            fetchedOts = fetchedOts.filter(ot => {
                ...
                if (searchTerm.trim() !== '') {
                    return ot.numero.toString().includes(searchTerm.trim());
                }

                if (showCompleted) {
                    // Mostrar SOLO Concluidas y Terminadas
                    return isRecent && (status === 'Concluida' || status === 'Terminada');
                } else {
    ...
    return (
        <div className="animate-fade mobile-view" style={{ paddingBottom: '4rem' }}>
            <div style={{ marginBottom: '2rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div>
                    <h1 style={{ fontSize: '1.25rem', fontWeight: '800' }}>Mis Servicios</h1>
                    <p style={{ color: 'var(--text-muted)', fontSize: '0.85rem' }}>Listado de OTs asignadas para atención.</p>
                </div>
            ...
            <div style={{ display: 'flex', gap: '0.75rem', marginBottom: '1.5rem' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', background: 'var(--bg-input)', padding: '0.5rem 0.75rem', borderRadius: '12px', border: '1px solid var(--glass-border)' }}>
                    <span style={{ fontSize: '0.6rem', fontWeight: '800', color: 'var(--text-main)' }}>FINALIZADAS</span>
                    <button
                                        ...
                                        <div key={ot.id} className="glass-card animate-fade" style={{ borderLeft: `4px solid var(--status-${ot.estatus.toLowerCase().replace(/ /g, '')})` }}>
                                            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.75rem' }}>
                                                <button
                                                    ...
                                                    style={{ background: 'transparent', border: 'none', padding: 0, cursor: 'pointer', fontSize: '0.7rem', fontWeight: '800', color: 'var(--text-muted)', textDecoration: 'underline' }}
                                                >
                                                    OT #{ot.numero}
                                                </button>
                                                <div style={{ display: 'flex', gap: '0.4rem', alignItems: 'center' }}>
                                                    {ot.prioridad && (
                                                        <span style={{
                                                            ...
                                                            borderRadius: '4px', background: ot.prioridad === 'ALTA' ? '#ef444420' : '#fbbf2420', color: ot.prioridad === 'ALTA' ? '#ef4444' : '#fbbf24'
                                                        }}>
                                                            {ot.prioridad}
                                                        </span>
                                                    )}
                                            ...
                                            <h3 style={{ fontSize: '1rem', fontWeight: '700', marginBottom: '0.25rem' }}>{suc?.nombre}</h3>
                                            <p style={{ fontSize: '0.8rem', color: 'var(--text-muted)', marginBottom: '0.75rem' }}>{suc?.direccion}</p>
                                            <p style={{ fontSize: '0.85rem', fontWeight: '600', color: 'var(--accent)' }}>{eq?.nombre}</p>
                                            {ot.descripcionFalla && (
                                                <div style={{ marginTop: '0.75rem', padding: '0.75rem', background: 'var(--bg-glass)', borderRadius: '8px', border: '1px solid var(--glass-border)' }}>
                                                    <p style={{ fontSize: '0.65rem', fontWeight: '800', color: 'var(--primary-light)', marginBottom: '0.25rem', textTransform: 'uppercase' }}>Descripción de la Falla:</p>
                                                    <p style={{ fontSize: '0.8rem', color: 'var(--text-muted)', lineHeight: '1.4' }}>{ot.descripcionFalla}</p>
                                                </div>
                                            )}

                                            <div style={{ marginTop: '1.25rem', paddingTop: '1.25rem', borderTop: '1px solid var(--glass-border)', display: 'flex', gap: '0.75rem' }}>
                                                {ot.estatus === 'Concluida' || ot.estatus === 'Terminada' ? (
                                                    <button
                                                        className="btn" style={{ flex: 1, padding: '0.875rem', fontSize: '0.85rem', background: 'var(--bg-glass)', color: 'var(--text-muted)', border: '1px solid var(--glass-border)' }}
                                                    ...
                                                    <button
                                                        className="btn btn-primary" style={{ flex: 1, padding: '0.875rem', fontSize: '0.85rem', background: 'var(--status-concluidapendientefirmacliente)' }}
                                                        onClick={() => navigate(`/ejecutar-servicio/${ot.id}`)}
                                                    >
                                                        <CheckCircle size={18} />
                                                        FIRMA CLIENTE
                                                    ...
                                                    <button
                                                        className="btn btn-primary" style={{ flex: 1, padding: '0.875rem', fontSize: '0.85rem' }}
                                                    ...
                                                    <button
                                                        className="btn btn-primary" style={{ flex: 1, padding: '0.875rem', fontSize: '0.85rem', background: 'var(--status-iniciada)' }}
                                                        onClick={() => navigate(`/ejecutar-servicio/${ot.id}`)}
                                                    >
                                                        <Camera size={18} />
                                                        CONTINUAR SERVICIO
                            ...
                            <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', minWidth: '400px' }}>
                                ...
                                <tbody>
                                    {[...ots].sort((a, b) => b.numero - a.numero).map(ot => {
                                        const prioColor = ot.prioridad === 'ALTA' ? '#ef4444' : (ot.prioridad === 'MEDIA' ? '#fbbf24' : 'var(--primary)');
                                            ...
                                            <tr key={ot.id} style={{ borderBottom: '1px solid var(--glass-border)', transition: 'background 0.2s' }} onMouseOver={e => e.currentTarget.style.background = 'rgba(255,255,255,0.02)'} onMouseOut={e => e.currentTarget.style.background = 'transparent'}>
                                                ...
                                                <td style={{ padding: '1rem', textAlign: 'right' }}>
                                                    <button
                                                        onClick={() => navigate(`/ejecutar-servicio/${ot.id}`)}
                                                        style={{
                                                            background: (ot.estatus === 'Concluida' || ot.estatus === 'Terminada') ? 'transparent' : 'var(--primary)',
                                                            border: `1px solid ${(ot.estatus === 'Concluida' || ot.estatus === 'Terminada') ? 'var(--glass-border)' : 'var(--primary)'}`,
                                                            color: (ot.estatus === 'Concluida' || ot.estatus === 'Terminada') ? 'var(--text-muted)' : 'white',
                                                            fontSize: '0.65rem', fontWeight: '800', padding: '0.4rem 0.8rem', borderRadius: '6px', cursor: 'pointer'
                                                        }}
                                                    >
                                                        {(ot.estatus === 'Concluida' || ot.estatus === 'Terminada') ? 'CONSULTAR' :
