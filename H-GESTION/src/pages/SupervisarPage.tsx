...
export const SupervisarPage: React.FC = () => {
    ...
    const fetchData = async () => {
        setLoading(true);
        try {
            // Only fetch Concluida for this page
            const q = query(
                collection(db, 'ordenesTrabajo'),
                where('estatus', '==', 'Concluida'),
                orderBy('numero', 'asc')
            );
            ...
            console.error(e);
            // If the composite index is missing, it will fail. Fallback to client-side filter if needed
            // But let's assume it works for now or I'd need to fetch all and filter
        }
    ...
    const handleFinalize = async (ot: WorkOrder) => {
        ...
        try {
            ...
            setSelectedOT(null);
            fetchData();
            showNotification("OT Terminada con éxito.", "success");
        } catch (e) {
            ...
            <div className="animate-fade">
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
                    <div>
                        ...
                        <p style={{ color: 'var(--text-muted)' }}>Órdenes terminadas por el técnico que requieren cierre definitivo.</p>
                    </div>
                </div>
                    ...
                    <div className="glass-card animate-scale-up" style={{ width: '100%', maxWidth: '1000px', maxHeight: '90vh', overflowY: 'auto', border: '2px solid #22c55e' }}>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '2rem' }}>
                                ...
                                <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                                    <section>
                                        <h3 style={{ fontSize: '0.9rem', fontWeight: '800', color: '#fbbf24', marginBottom: '0.5rem' }}>REPUESTOS Y MATERIALES</h3>
                                        <p style={{ background: 'rgba(0,0,0,0.2)', padding: '1rem', borderRadius: '12px', fontSize: '0.9rem' }}>{selectedOT.repuestosUtilizados || 'N/A'}</p>
                                    </section>
                                    <section>
                                        <h3 style={{ fontSize: '0.9rem', fontWeight: '800', color: 'var(--primary)', marginBottom: '0.5rem' }}>EVIDENCIA FOTOGRÁFICA</h3>
                                        ...
                                        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.4rem', maxHeight: '150px', overflowY: 'auto', background: 'var(--bg-input)', padding: '0.5rem', borderRadius: '8px' }}>
                                            {bitacora.map(entry => (
                                                <div key={entry.id} style={{ fontSize: '0.65rem', borderBottom: '1px solid rgba(255,255,255,0.05)', paddingBottom: '0.25rem' }}>
                                                    <span style={{ color: '#fbbf24' }}>{entry.accion}</span> - {new Date(entry.fecha).toLocaleString()}
