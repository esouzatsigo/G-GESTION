...
export const EjecucionServicioPage: React.FC = () => {
    ...
    const handleTecnicoClosure = async () => {
        ...
        try {
            ...
            // Navegar de regreso a la lista para forzar la actualización del estatus visual
            // o permitir que el técnico proceda con la firma del cliente en esta misma vista
            // (el estado local 'updatedOT' ya tiene el nuevo estatus, bloqueando los campos)
        } catch (error) {
            console.error(error);
    ...
    const handleFinalClosure = async () => {
        ...
        try {
            ...
            if (user) {
                // AQUÍ ES DONDE PASA A CONCLUIDA DEFINITIVAMENTE
    ...
    return (
        <div className="animate-fade mobile-view">
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.6rem', marginBottom: '0.75rem', flexWrap: 'nowrap' }}>
                ...
                <div style={{ display: 'flex', gap: '5px', flexShrink: 0 }}>
                    <button
                        onClick={() => {
                            let query = '';
                            // Prioridad 1: Coordenadas Satelitales (Precisión Total)
                            if (sucursal?.coordenadas?.lat && sucursal?.coordenadas?.lng) {
                                query = `${sucursal.coordenadas.lat},${sucursal.coordenadas.lng}`;
                            } 
                            // Prioridad 2: Dirección Postal Registrada
                            else if (sucursal?.direccion) {
                                query = sucursal.direccion;
                            } 
                            // Prioridad 3: Nombre de la Sucursal (Fallback)
                            else {
                                query = ot.sucursalNombre || '';
                            }
            ...
            <div className="glass-card" style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem', padding: '0.75rem' }}>
                ...
                <div style={{ padding: '0.4rem 0.75rem', background: 'rgba(255,193,7,0.05)', borderRadius: '12px', border: '1px solid rgba(255,193,7,0.15)', display: 'flex', flexDirection: 'column', gap: '0.25rem', alignItems: 'center', textAlign: 'center' }}>
                    <AlertCircle size={16} color="#ffc107" />
                    <p style={{ fontSize: '0.7rem', color: '#ffc107', fontWeight: '600', lineHeight: '1.2', margin: 0 }}>
                        Estas fotos son la base del reporte PDF. Asegúrate de que sean claras.
                    </p>
                </div>
                ...
                <div style={{ padding: '1rem', background: 'rgba(255,255,255,0.02)', borderRadius: '16px', border: '1px solid var(--glass-border)', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                    ...
                    {ot.estatus !== 'Concluida. Pendiente Firma Cliente' && ot.estatus !== 'Concluida' && ot.estatus !== 'Finalizada' && (
                        <div style={{ padding: '0.75rem', background: '#0f172a', borderRadius: '16px', border: '1px solid #1e293b', display: 'flex', flexDirection: 'column', gap: '0.5rem', alignItems: 'center', boxShadow: '0 4px 15px rgba(0,0,0,0.4)', textAlign: 'center' }}>
                            <div style={{ background: 'rgba(59, 130, 246, 0.1)', padding: '0.4rem', borderRadius: '50%', width: 'fit-content' }}>
                                <AlertCircle size={22} color="#3b82f6" />
                            </div>
                            <p style={{ fontSize: '0.75rem', color: '#f8fafc', fontWeight: '400', lineHeight: '1.4', margin: 0 }}>
                                <span style={{ color: '#3b82f6', fontWeight: '900', display: 'block', fontSize: '0.9rem', marginBottom: '0.1rem' }}>COMPROMISO DE CALIDAD</span>
                                Tu cierre marcará hora y ubicación. <strong style={{ color: '#3b82f6' }}>Inamovible tras este paso.</strong>
                            </p>
                        </div>
                    )}
